...
class IRCServer(object):
   ...
   # TODO: Write the code required when the server has a message to be sent to another server
    ...
    def send_message_to_server(self, name_of_server_to_send_to, message):
        # look up the server to send to in the lookup table and then put the message in the writebuffer of theat
        # server. The selector will handle all of the these write events.
        # keep in mind first_link will need to be used to send msgs to non-adjacent servers.
        print("send_message_to_server called: ", name_of_server_to_send_to)
        self.servers_lookuptable[name_of_server_to_send_to].write_buffer += message  # use plus= so that multiple msgs can be in the buffer
    ...
    def send_message_to_client(self, name_of_client_to_send_to, message):
        # client_obj = self.users_lookuptable[name_of_client_to_send_to]
        # if name_of_client_to_send_to in self.adjacent_users:
        #     _obj.write_buffer += message
        # else:
        #     self.send_message_to_server(client_obj.first_link, message)
           
    
        c_name = name_of_client_to_send_to
        if c_name in self.adjacent_users:
            self.users_lookuptable[c_name].write_buffer += message
        else:
            s_name = self.users_lookuptable[c_name].firstlink
            self.send_message_to_server(s_name, message)

    ####BELOW HERE IS YOUR OLD CODE
        # for user_obj in self.users_lookuptable.values():
        #     for adj_user in self.adjacent_servers:
        #         if(user_obj.nick == adj_user):
        #             user_obj.write_buffer = message
        #         else:
        #             self.send_message_to_server(self.users_lookuptable[name_of_client_to_send_to].first_link, message)
        #             # self.users_lookuptable.get(
        #             #     user_obj.first_link).write_buffer += message
    ...
    def handle_user_message(self, select_key, prefix, command, params):
        ...
        if len(params) < 4:
            # :server_name ### <command> :Not enough parameters
        ...
        if not prefix:
            ####
            #this had to be moved below the store to lookuptable for order of operations
            # :server_name ### :Welcome to the Internet Relay Network <nick>!<user>@<host>
    ...
    def handle_server_message(self, select_key, prefix, command, params):
        # server msgs will always have 3 params
        ...
        if(len(params) == 3):
            ...
            self.create_numeric_reply(
                "ERR_NEEDMOREPARAMS", "The message is missing parameters")

        self.servers_lookuptable.update({serverData.servername: serverData})
        print("Handle server message - This is the lookup table",
              self.servers_lookuptable)

        print("Handle server message - There is no prefix in this message")
        msg = ":{0} SERVER {1} {2} :{3}\r\n".format(
            self.servername, params[0], str(int(params[1])+1), params[2])
        self.broadcast_message_to_servers(msg, serverData.first_link)

        print(" these are the adjacent servers",
              self.adjacent_servers)

        if(params[1] == "1"):
            print("prefix detected and it matches the the server name")
            self.adjacent_servers.append(params[0])
            msg = ":{0} SERVER {1} {2} :{3}\r\n".format(
                self.servername, params[0], params[1], params[2])

        # If not prefix, update new server with this server's stored servers, users, and channels
        # AND ALL THE USERS AND CHANNELS!!!
        if not prefix:
            for data in self.servers_lookuptable.values():
                print("this is the server sending the message: ", self.servername)
                print("this is the data.servername:", data.servername)
            ...
            # Need to create user msg and channel msg; send these msgs to associated servers.
    ...
    def handle_quit_message(self, select_key, prefix, command, params):
        ...
        if not prefix:
            ...
            # if len(params) == 1:
            #     quit_msg = command + " " + params[0] + "\r\n"
            #     self.broadcast_message_to_servers(
            #         quit_msg, self.servername)
            # else:
            #     quit_msg = command + " " + select_key.data.nick + " has quit\r\n"
            #     self.broadcast_message_to_servers(
            #         quit_msg, self.servername)
        else:
            user = prefix
